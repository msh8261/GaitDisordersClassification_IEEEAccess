...
def get_mi_vector(MI_FS, F, s):
    ...
    MIs = Parallel(n_jobs=MI_FS.n_jobs)(delayed(_get_mi)(f, s, MI_FS) for f in F)
    return MIs
...
def get_first_mi_vector(MI_FS, k):
    ...
    """
    n, p = MI_FS.X.shape
    MIs = Parallel(n_jobs=MI_FS.n_jobs)(
        delayed(_get_first_mi)(i, k, MI_FS) for i in range(p)
    )
    return MIs
...
def _mi_cc(variables, k=1):
    ...
    return sum([_entropy(X, k=k) for X in variables]) - _entropy(all_vars, k=k)
