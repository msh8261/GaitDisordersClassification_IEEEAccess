"""'
git: https://github.com/DavidHarar/FSM
blog : https://towardsdatascience.com/feature-selection-for-the-lazy-data-scientist-c31ba9b4ee66
...
import prepare_dataset.best_features_selection.mifs as mifs
from prepare_dataset.best_features_selection.utils_feature_selections import *


def mRMR(X, y):
    """pip install mrmr_selection
    which is a feature selection algorithm
    stands for "minimum Redundancy - Maximum Relevance"
    """
...
class FSM:
    ...
    def __init__(self, k, filler):
        ...
        self.ReliefF_selector = skrebate.ReliefF(
            n_features_to_select=k, n_neighbors=100, verbose=True
        )
        ...
        self.MultiSURF_selector = skrebate.MultiSURF(
            n_features_to_select=k, verbose=True
        )
        self.jmi = mifs.MutualInformationFeatureSelector(
            method="JMI", n_jobs=use, n_features=k
        )
        self.jmim = mifs.MutualInformationFeatureSelector(
            method="JMIM", n_jobs=use, n_features=k
        )
    ...
    def Bootstrapper(self, X, y, B, Sample_size):
        ...
        X = X.copy()
        # df = pd.DataFrame(data=X[1:, 1:], index=X[1:, 0], columns=X[0, 1:])
